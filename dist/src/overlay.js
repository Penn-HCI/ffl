...
function isAdjacent(a, b, maxGap) {
...
function groupByAdjacent(elements) {
    let ret = [];
    if (elements.length > 0) {
        ret.push([elements[0]]);
        for (var i = 1; i < elements.length; i++) {
...
function drawBackground(backgroundInfo, root) {
    ...
    try {
        ...
        root.style.position = 'relative';
        backgroundInfo.forEach(({ selector, backgroundColor }) => groupByAdjacent([...root.querySelectorAll(selector).values()].sort((a, b) => {
            let ba = a.getBoundingClientRect(), bb = b.getBoundingClientRect();
            return ba.left - bb.left || ba.top - bb.top;
        })).map(group => {
            ...
            let bgRectDim = utils_1.BoundingBox.of(...group.map(node => new utils_1.BoundingBox(node.getBoundingClientRect()))).relativeTo(rootBoundingBox);
