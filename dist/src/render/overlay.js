...
import { INSTANCE_DATA_ATTR, toSelectorStrings } from '../../ffl';
...
function drawLabelGroup(labelInfo, root, rootBoundingBox, direction) {
    ...
    nodes.forEach((node, idx) => {
        ...
        path.setAttribute('d', rebasedPath(node.data.symbolBoundingBox.center.horizontal, base + (labelInfo[idx].markerOffset?.y ?? 0)));
        ...
        if (labelInfo[idx].labelMarker === 'extent') {
            ...
            brace.setAttribute('transform', `translate(${labelInfo[idx].markerOffset?.x ?? 0}, ${anchorLineY + (labelInfo[idx].markerOffset?.y ?? 0)})`);
...
export function drawLabels(labels, root, scopeKey) {
    ...
    try {
        ...
        let labelInfo = labels.flatMap(({ selectorInfo, label, labelPosition, labelMarker, markerOffset }) => toSelectorStrings(selectorInfo, scopeKey).map((ss, idx) => ({
            selector: ss,
            ...
            return {
                symbolBoundingBox: elements !== undefined ? BoundingBox.of(...elements.map(node => new BoundingBox(node.getBoundingClientRect())))?.relativeTo(rootBoundingBox) : undefined,
        ...
        let [top, bottom] = partition(labelInfo, info => (info.labelPosition && info.labelPosition !== 'auto') ? info.labelPosition === 'above'
...
export function drawBackground(backgroundInfo, root, scopeKey) {
    // need to make sure element is rendered to find the bounding box
    let visibility = setVisible(root);
    try {
        let rootBoundingBox = new BoundingBox(root.getBoundingClientRect());
        ...
        root.style.position = 'relative';
        backgroundInfo.flatMap(({ selectorInfo, backgroundColor }) => toSelectorStrings(selectorInfo, scopeKey).map((ss, idx) => ({
            selector: ss,
            classes: selectorInfo[idx].selectors.map(s => s.class),
            backgroundColor
            ...
            let bgRectDim = BoundingBox.of(...group.map(node => new BoundingBox(node.getBoundingClientRect()))).relativeTo(rootBoundingBox);
...
export function drawBorders(backgroundInfo, root, scopeKey) {
    // need to make sure element is rendered to find the bounding box
    let visibility = setVisible(root);
    try {
        let rootBoundingBox = new BoundingBox(root.getBoundingClientRect());
        ...
        root.prepend(overlay);
        root.style.position = 'relative';
        backgroundInfo.flatMap(({ selectorInfo, border }) => toSelectorStrings(selectorInfo, scopeKey).map((ss, idx) => ({
            selector: ss,
            classes: selectorInfo[idx].selectors.map(s => s.class),
            border
